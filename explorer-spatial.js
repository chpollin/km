...
class SpatialManager {
    ...
    createTemporalLayout() {
        ...
        const sortedObjects = [...this.explorer.objects].sort((a, b) => {
            const dateA = this.extractYear(a.createdDate) || 1900;
            const dateB = this.extractYear(b.createdDate) || 1900;
            return dateA - dateB;
        });
        
        // Position along timeline
        sortedObjects.forEach((obj, index) => {
            const year = this.extractYear(obj.createdDate) || 1900;
            const normalizedYear = (year - 1880) / (1930 - 1880); // Normalize to 0-1
            
            obj.x = -2000 + normalizedYear * 4000;
            obj.y = -1000 + (index % 100) * 20 + (Math.random() - 0.5) * 40;
        });
    }

    createGridLayout() {
        const columns = Math.ceil(Math.sqrt(this.explorer.objects.length));
        const spacing = 80;
        
        this.explorer.objects.forEach((obj, index) => {
            const row = Math.floor(index / columns);
            const col = index % columns;
            
            obj.x = -2000 + col * spacing;
            obj.y = -1500 + row * spacing;
        });
    }

    createSimilarityLayout() {
        // Use a simplified similarity calculation
        const clusters = this.createSimilarityClusters();
        ...
        clusters.forEach((cluster, clusterIndex) => {
            ...
            this.positionCluster(cluster, centerX, centerY, 'circular');
        });
    }

    createSimilarityClusters() {
        // Simple clustering based on title keywords and type
        const clusters = [];
        const used = new Set();
        
        this.explorer.objects.forEach(obj => {
            if (used.has(obj.identifier)) return;
            
            const cluster = [obj];
            used.add(obj.identifier);
            
            // Find similar objects
            const keywords = this.extractKeywords(obj.title || '');
            
            this.explorer.objects.forEach(other => {
                if (used.has(other.identifier)) return;
                if (obj.container !== other.container) return;
                
                const otherKeywords = this.extractKeywords(other.title || '');
                const similarity = this.calculateKeywordSimilarity(keywords, otherKeywords);
                
                if (similarity > 0.3 && cluster.length < 50) {
                    cluster.push(other);
                    used.add(other.identifier);
                }
            });
